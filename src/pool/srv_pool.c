...
#if 0
static void
event_timer(void *arg)
{
	struct pool_svc_events *events = arg;

	while (!sched_req_is_aborted(events->pse_timer)) {
		int64_t time_left;

		ABT_mutex_lock(events->pse_mutex);
		time_left = events->pse_time + events->pse_timeout - daos_gettime_coarse();
		if (time_left <= 0) {
...
events_handler(void *arg)
{
	...
	for (;;) {
		...
		for (;;) {
			...
			/* A timed cond_wait without polling. */
			if (time_left > 0) {
