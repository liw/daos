...
pool_properties(void **state)
{
	...
	test_set_fail_loc(arg, info.pi_leader, DAOS_FORCE_PROP_VERIFY | DAOS_FAIL_ALWAYS);
	par_barrier(PAR_COMM_WORLD);
...
pool_op_retry(void **state)
{
	...
	test_set_fail_loc(arg, arg->pool.pool_info.pi_leader,
	...
	test_set_fail_loc(arg, info.pi_leader, DAOS_POOL_QUERY_FAIL_CORPC | DAOS_FAIL_ONCE);
	...
	test_set_fail_loc(arg, info.pi_leader, DAOS_POOL_DISCONNECT_FAIL_CORPC | DAOS_FAIL_ONCE);
