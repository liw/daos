...
pool_create_and_destroy_retry(void **state)
{
	...
	test_set_fail_loc(arg, CRT_NO_RANK, DAOS_POOL_CREATE_FAIL_CORPC | DAOS_FAIL_ONCE);
	print_message("creating pool synchronously ... ");
	rc = dmg_pool_create(dmg_config_file,
			     geteuid(), getegid(),
			     arg->group, NULL /* tgts */,
...
pool_create_steps_down_from_up_empty(void **state)
{
	...
	test_set_fail_loc(arg, CRT_NO_RANK, DAOS_POOL_CREATE_FAIL_STEP_UP | DAOS_FAIL_ONCE);
