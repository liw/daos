...
static void
crt_ctx_epi_mark(struct crt_ep_inflight *epi, bool dead, int ctx_idx)
{
...
crt_rank_epi_op(d_rank_t rank, enum crt_rank_epi_op_type type)
{
	...
	for (i = 0; i < epis.p_len; i++) {
		...
		if (type == CRT_RANK_EPI_ABORT || type == CRT_RANK_EPI_MARK_DEAD_ABORT) {
			...
			crt_ctx_epi_mark(epi, false /* dead */, ctx->cc_idx);
		} else {
...
crt_context_req_track(struct crt_rpc_priv *rpc_priv)
{
	...
	if (epi->epi_dead) {
		RPC_TRACE(DB_NET, rpc_priv, "cancel due to dead destination rank\n");
		/*
		 * Return -DER_TIMEDOUT to be consistent with crt_req_abort and
		 * to simplify users' error handling.
		 */
		D_GOTO(out_unlock_epi, rc = -DER_TIMEDOUT);
	}
