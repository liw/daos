...
func SystemProperties() SystemPropertyMap {
	...
	return SystemPropertyMap{
		...
		SystemPropertySelfHeal: SystemProperty{
			...
			Value:       NewStringPropVal("exclude;pool_exclude;pool_rebuild", enumSubsets([]string{"exclude", "pool_exclude", "pool_rebuild"}, ";", "empty")...),
...
// set represented by emptySet.
func enumSubsets(strings []string, sep string, emptySet string) []string {
	if len(strings) == 0 {
		return []string{emptySet}
	}

	subsets := enumSubsets(strings[1:], sep, emptySet)
	n := len(subsets)
